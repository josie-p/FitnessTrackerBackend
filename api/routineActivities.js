...
router.patch("/:routineActivityId", requireUser, async (req, res, next) => {
  ...
  const findRoutineActivityForName = await getRoutineActivityById(
    routineActivityId
  ); //getting name of routine for error message
  const findName = await getRoutineById(findRoutineActivityForName.routineId);
  const name = findName.name;

  try {
    const findRoutineActivity = await canEditRoutineActivity(
      routineActivityId,
      id
    );
